...
process_files_in_directory() {
    ...
    for file in "${all_files[@]}"; do
        ...
        # Exclude files ending in -sardonic.txt, -overview.txt, and -critique.txt
        if [[ "$file" =~ -sardonic\.txt$ || "$file" =~ -overview\.txt$ || "$file" =~ -critique\.txt$ ]]; then
        ...
        if ! is_processed "$file"; then
            ...
            local summary_file="$dir/${file_name%.txt}-summary.txt"
            touch "$summary_file"
